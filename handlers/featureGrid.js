...
export async function createOrUpdateFeatureGrid(env, blockData, assetMap = null, summary = null) {
    ...
    if (link.url || blockData.label25) {
        ctaEntry = await upsertCta(env, blockId, blockData.label25 || link.label || "", link.url);
    }
    ...
    for (const [gId, grid] of Object.entries(gridData)) {
        ...
        const itemTitle = await upsertSectionTitle(env, `fg-${gId}`, f.heading || "");

        let itemCta = null;
        const itemLink = parseCraftLink(f.ctaLink);
        if (itemLink.url || f.ctaLabel) {
            itemCta = await upsertCta(env, `fg-${gId}`, f.ctaLabel || itemLink.label || "", itemLink.url);
        }
        ...
        const itemEntry = await upsertEntry(env, "featureGridItem", `fgitem-${gId}`, itemFields);
    ...
    if (blockData.subheadingSmall) {
        fields.description = { [LOCALE]: await convertHtmlToRichText(env, blockData.subheadingSmall) };
    }
