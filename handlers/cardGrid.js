...
export async function createOrUpdateCardGrid(env, blockData, assetMap = null) {
    ...
    for (const [gId, group] of Object.entries(cardGridData)) {
        ...
        for (const [cId, card] of Object.entries(cards)) {
            ...
            const itemTitle = await upsertSectionTitle(env, `cg-${cId}`, f.description || "");

            let ctaEntry = null;
            const link = parseCraftLink(f.ctaLink);
            if (link.url || f.ctaLabel) {
                ctaEntry = await upsertCta(env, `cg-${cId}`, f.ctaLabel || link.label || "", link.url);
            }
            ...
            const itemEntry = await upsertEntry(env, "iconGridItem", `cgitem-${cId}`, itemFields);
    ...
    const fields = {
        ...
        cardTheme: { [LOCALE]: THEME_MAP[(blockData.cardTheme || "").toLowerCase()] || "Light" }
    };
    if (titleEntry) fields.sectionTitle = { [LOCALE]: makeLink(titleEntry.sys.id) };
    if (blockData.subheading) fields.description = { [LOCALE]: blockData.subheading };
