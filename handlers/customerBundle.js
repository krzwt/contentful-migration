...
export async function createOrUpdateCustomerBundle(env, blockData, assetMap = null) {
    ...
    for (const [gId, group] of Object.entries(bundleData)) {
        ...
        for (const [iId, item] of Object.entries(gridItems)) {
            ...
            if (link.url) {
                ctaEntry = await upsertCta(env, `cb-${iId}`, link.label || "", link.url);
            }
            ...
            const itemEntry = await upsertEntry(env, "customerBundleItem", `cbitem-${iId}`, itemFields);
    ...
    const fields = {
        ...
        blockName: { [LOCALE]: blockData.blockName || heading || "Customer Bundle" }
    };
