...
const LOCALE = "en-US";
const CONTENT_TYPE = "contentCta";

/**
 * Handler for contentCta (nested in contentBlock as type: cta)
 */
export async function createOrUpdateContentCta(env, id, fields, summary = null) {
    try {
    ...
    if (rawLink) {
        ...
        if (url || label || linkInfo.linkedId) {
            ctaEntry = await upsertCta(env, `contentcta-${id}`, label, url, true, linkInfo.linkedId);
        }
    }
    ...
    return await upsertEntry(env, CONTENT_TYPE, `contentcta-${id}`, cfFields);
}
