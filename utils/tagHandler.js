...
export function loadTagMapping(filePath = "./data/tags.json") {
  ...
  try {
    ...
    const tags =
      data.data?.tags || data.tags || (Array.isArray(data) ? data : []);
...
export async function processTags(env, craftTagIds) {
  ...
  for (const id of craftTagIds) {
    ...
    if (!tag) {
      ...
      console.log(
        `   ✨ Creating new Environment Tag: "${tagName}" (${tagId})`,
      );
      try {
...
/**
 * Helper to get raw tag names for join operation
 */
export function getTagNames(craftTagIds) {
  if (!craftTagIds || !craftTagIds.length) return [];
  return craftTagIds.map((id) => tagMapping.get(String(id))).filter(Boolean);
}
